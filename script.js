...
// });
// // log into binance to activate your api key. choose key with least privalage possible

// const symbol = "SOL/USD";
// const type = "limit";
// const side = "buy";
// const amount = 5; //here you chose how many solana you want to buy. Can change value.
...
// you can test your code with fake money on Binance / back testing.
...
const run = async () => {
  try {
    let res = await fetch(apiUrlHist, {
      headers: {
        "Content-Type": "application/json",
        Accept: "application/json",
      },
    ...
    resJson.prices.pop();
    const average =
      resJson.prices.reduce((sum, el) => sum + el[1], 0) /
      resJson.prices.length;
    ...
    if (currentPrice > average) {
      ...
      console.log(
        `Buy order created. ${amount} ${symbol} - Limit @ ${limitPrice} - Take profit @ ${params.takeProfit.triggerPrice} - Stop loss @ ${params.stopLoss.triggerPrice}`
      );
      console.log(order);
    }
  } catch (error) {
    console.error("Error fetching data or placing order:", error);
  }
};

setInterval(run, 86400 * 1000); // Run the function once every 24 hours
